...
public class LivroService {
    ...
    public LivroEntity buscarPorIsbn(String isbn) {
    ...
    public void deletarPorTitulo(String titulo) {
        List<LivroEntity> livros = livroRepository.findByTituloContaining(titulo);

        if (livros.isEmpty()) {
            throw new RuntimeException("Nenhum livro encontrado com esse título: " + titulo);
        }

        for (LivroEntity livro : livros) {
            livroRepository.deleteByTitulo(livro.getTitulo());
        }
    }
